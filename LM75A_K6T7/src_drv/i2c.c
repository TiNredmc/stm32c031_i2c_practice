...
#define LM75_ADDR			0x48 // I2C 7-bit address
#define LM75_TEMPREG	0x00 // Temperature readout register
...
void i2c_readPtr(
	...
	while(1){
		switch(readFSM){
			...
			case 1:// Wait until RX is not empty after slave address write
			{
			...
			case 2:// Read data and save to RX buffer
			{
				*ptr++ = I2C1->RXDR;
				len--;
				nbyte--;
...
uint16_t temp;
	...
	// Swap MSbyte and LSbyte (Data sent from sensor is Big-endian).
	temp = (temp << 8) | (uint8_t)(temp >> 8);
	
	return ((int16_t)(temp >> 5) * 0.125);
}